...
namespace CourtCheckInPrism.Views
{
    public partial class VisitSchedule : ContentPage
    {
        ...
        public VisitSchedule()
        {
            ...
            var schedule1 = (from sch in conn.Table<CourtScheduleModel>() where sch.CheckInTime == null && sch.DateOfCourtAppearence >= DateTime.Today orderby sch.CheckInTime select sch);
            var schedule2 = (from sch in conn.Table<CourtScheduleModel>() where sch.CheckInTime != null && sch.Testify == null orderby sch.CheckInTime select sch);
